...
def sclearn_run (name, metric, runs, queries, method) :
    print(method + " Sclearn start ----------------------------------------------")
    ...
    def createIndex(indexMethod, datasetImages):
        ...
        stopIndex = indexMethod(algorithm= method, metric='euclidean').fit(datasetImages)
        time_end = perf_counter()
        totalTime = (time_end - time_start)
        return (stopIndex, totalTime)
    
    (minBuildTime, maxBuildTime, indexedStruct) = createIndexNumerous(createIndex, NearestNeighbors, datasetTrainImages, runs)
